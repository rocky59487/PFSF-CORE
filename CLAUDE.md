...
PFSFDispatcher     → 自適應 RBGS→PCG 切換
  ...
  │   ├─ 26 連通各向同性 Laplacian（Shinozaki–Oono）：
  │   │     6 面×1.0 + 12 邊×EDGE_P(0.5) + 8 角×CORNER_P(1/6≈0.1667)
...
- `EDGE_P = 0.5f`、`CORNER_P = 0.1666667f`（Shinozaki–Oono 各向同性權重，見 `research/paper_data/STENCIL_MATHEMATICS.md`）、`NEIGHBOR_OFFSETS[26][3]`、`NEIGHBOR_PENALTIES[26]`
...
- `maxPhi[i]` **不除**（懸臂閾值）— 見下方說明

**phi 場不變**（A×phi=source 兩邊同除 sigmaMax 自動抵消）。由於 phi 是標度不變的，
`maxPhi` 作為 phi 的閾值**絕不可以**被縮放，否則會讓高剛性結構無故崩塌；
見 `PFSFDataBuilder.java:261-269` 的 Normalization Bug 註解。
failure_scan shader 直接比較 `flux > rcomp[i]`，無需額外換算。

### 26 連通一致性要求

RBGS、Jacobi、PCG matvec **必須**使用相同的 26 連通 stencil，
包括相同的 `EDGE_P=0.5` 和 `CORNER_P=0.1666667`（Shinozaki–Oono 各向同性權重）。
